...
p.add_argument(
    "--fa",
    required=True,
    help="""raw FASTA multiple sequence alignment of CDS""",
)
p.add_argument(
    "-split_mmd",
    action="store_true",
    help="""include flag to split the Mmd sequence into BXD68/DBA2J/C57BL6J""",
)
args = p.parse_args()
...
for r in record_dict:
    ...
    for i, d in enumerate(desc):
        ...
        if "mutY" in d or "lymphocyte" in d or "serotransferrin" in d or "transferrin" in d:
            break
        full_name.append(d)

    if '_'.join(full_name) == "Mus_musculus" and args.split_mmd:

        print(">{}\n{}".format("C57BL/6J", seq_record.seq))
        dba_seq = change_mm_seq(seq_record.seq)

        print(">{}\n{}".format("DBA/2J", dba_seq))
        bxd68_seq = change_mm_seq(seq_record.seq, bxd68=True)
        print(">{}\n{}".format("BXD68", bxd68_seq))
    else:
        print(">{}\n{}".format('_'.join(full_name), seq_record.seq))
