...
mut2idx = dict(
    zip(pd.unique(dumont['kmer']), range(len(pd.unique(dumont['kmer'])))))
...
for mut in mut2idx:
    ...
    ax.scatter(x, y, color=color, edgecolor=edgecolor, label=label, s=100)
...
ax.set_ylabel(
    'Log-2 ratio of mutation fractions in DBA-like\nvs.C57-like MGP strains')
ax.set_xlabel(
    'Log-2 ratio of mutation fractions in BXDs\nwith D vs. B haplotypes at QTL'
)
sns.set_style('ticks')
