...
def run(args):
    ...
    isogenic_smps = [
        "4512-JFI-0348_BXD24_TyJ_Cep290_J_phased_possorted_bam",  # iso with BXD24
    ...
    for i, v in enumerate(vcf_h):
        ...
        for alt_idx, alt in enumerate(v.ALT):
            ...
            singleton = get_singleton_idx(gts_reformatted,
                                          ad,
                                          rd,
                                          ab_thresh=0.9)
            ...
            for haplotype_fh in args.haplotypes:
                ...
                hap_tree = make_interval_tree(haplotype_fh,
                                              datacol=True,
                                              delim=',')
                singleton_founder_hap = hap_tree[str(v.CHROM)].search(
                    v.start, v.end)

            # make sure we've got a legit haplotype here
            if singleton_founder_hap is None or len(
                    singleton_founder_hap) == 0:
                continue
            ...
            for mouse in samples_in_vcf:
                ...
                if not (gq[m_idx] >= args.min_gq and td[m_idx] >= args.min_dp):
                    continue
                samp_with_hap_hr += 1
            ...
            outvals = [
                ...
                bxd_line,
                kmer,
                singleton_founder_hap,
                gts_reformatted[singleton],
                td[singleton],
                ab[singleton],
                con_score,
            ]
